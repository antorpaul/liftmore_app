...
export const fetchAllExercises = async (page: number = 0, page_size: number = 10, category_id: number = -1) : Promise<Exercise[]> => {
  try {
      const response = await fetch(`${urlBase.base}${urlBase.exercises}/all?page=${page}&page_size=${page_size}&category_id=${category_id}`);
